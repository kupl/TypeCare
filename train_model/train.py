...
def train(train_datas, is_return=False):
    ...
    name = "random_forest_model.pkl" if not is_return else "ret_random_forest_model.pkl"
...
def run():
    ...
    # check file data
    if not recreate_data and os.path.isfile(DATA_SAVE_PATH / "train_data.parquet"):
        train_datas = pd.read_parquet(DATA_SAVE_PATH / 'train_data.parquet', engine='pyarrow')
        ret_train_datas = pd.read_parquet(DATA_SAVE_PATH / 'ret_train_data.parquet', engine='pyarrow')
    else:
        ...
        train_datas.to_parquet(DATA_SAVE_PATH / 'train_data.parquet', index=False)
        ret_train_datas.to_parquet(DATA_SAVE_PATH / 'ret_train_data.parquet', index=False)
        train_datas = pd.read_parquet(DATA_SAVE_PATH / 'train_data.parquet', engine='pyarrow')
        ret_train_datas = pd.read_parquet(DATA_SAVE_PATH / 'ret_train_data.parquet', engine='pyarrow')
    ...
    test_repo_path = "BetterTypes4Py" / "repos" / "test"
    ...
    if not recreate_data and os.path.isfile(DATA_SAVE_PATH / "test_data.parquet"):
        ...
        test_datas.to_parquet('test_data.parquet', index=False)
