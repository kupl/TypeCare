...
def process_prediction(args):
    ...
    if index is None:
        ...
        try:
            ...
            p = subprocess.Popen(
                ['pyright', '--outputjson', tiger_path / copy_path],
...
def run():
    ...
    pred_path = Path("output/predictions")
    testset_path = Path("data/ManyTypes4Py-JSON") / "new_testset.json"

    with open(pred_path / "randomsampled.json", "r") as f:
        tiger_result = json.load(f)

    with open(testset_path, "r") as f:
        testset = json.load(f)

    user_types_dict = json.load(open(Path("data/ManyTypes4Py-JSON") / "testset_usertypes.json", "r"))
    ...
    for test_info in testset:
        file_path = test_info['file']
        proj_path = "/".join(file_path.split("/")[:3])

        try:
            contexts = test_info['loc']
            var_name = test_info['name']
            kind = test_info['scope']

            repo = file_path + "--" + contexts + "--" + var_name + "--" + kind
            if repo not in tiger_result:
                continue
            ...
            if kind in ["arg", "return"]:
                code_annotation = test_info["code_annotation"]
                ...
                generating_preds, ranking_preds, all_cands, likelihoods, similarities =\
                    tiger_result[repo]["generating"], tiger_result[repo]["ranking"], tiger_result[repo]["candidates"], tiger_result[repo]["likelihoods"], tiger_result[repo]["similarities"]

                user_types = set(user_types_dict[repo][1])
                valid_set = set()
                ...
                predictions = [pred for (pred, _, _, _) in final_preds][:10]  # Get top 10 predictions
                ...
                try:
                    ...
                    for index, pred in enumerate(predictions):
                        datas.append(
                            (index, pred, tiger_path, proj_path, result_path, file_path, class_name, method_name, var_name, kind)
                        )

                    # add for removed code
                    datas.append(
                        (None, code_annotation, tiger_path, proj_path, result_path, file_path, class_name, method_name, var_name, kind)
                    )
                ...
                file_time_dict[repo] = round(file_end_time - file_start_time, 2)
    ...
    print(f"Processed {len(testset)} test cases.")
